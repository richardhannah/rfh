#!/usr/bin/env pwsh
# Test workflow with existing publisher user
...
## Example
``````
When user asks "add validation rules", apply this rule to ensure proper structure.
``````
...
Write-Host "`nSearching for 'rules'..." -ForegroundColor Gray
& $RfhBinary search "rules"
